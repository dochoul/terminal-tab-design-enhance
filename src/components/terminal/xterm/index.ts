...
export class Xterm {
    ...
    private onDirectoryChange?: (directory: string) => void;
    ...
    public open(parent: HTMLElement) {
        ...
        if (viewport) {
            viewport.style.setProperty('--sb-thumb', 'transparent');
            parent.addEventListener('mouseenter', () => viewport.style.setProperty('--sb-thumb', 'rgba(255,255,255,0.25)'));
            parent.addEventListener('mouseleave', () => viewport.style.setProperty('--sb-thumb', 'transparent'));
        }
    }

    @bind
    private detectWorkingDirectory(data: Uint8Array): string | null {
        const text = this.textDecoder.decode(data, { stream: false });

        const bashPromptMatch = text.match(/[\w-]+@[\w-]+:([~\w\/.-]+)[\$#]\s*$/);
        if (bashPromptMatch) {
            return bashPromptMatch[1];
        }

        const zshPromptMatch = text.match(/[\w-]+@[\w-]+\s+([~\w\/.-]+)\s*[>%#]\s*$/);
        if (zshPromptMatch) {
            return zshPromptMatch[1];
        }

        const simplePathMatch = text.match(/([~\w\/.-]+)[\$#]\s*$/);
        if (simplePathMatch) {
    ...
    public writeData(data: string | Uint8Array) {
        ...
        const newDir = this.detectWorkingDirectory(data);
        if (newDir && this.onDirectoryChange) {
            this.onDirectoryChange(newDir);
        }
